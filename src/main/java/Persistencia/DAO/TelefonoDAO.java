...
public class TelefonoDAO implements ITelefonoDAO {
    ...
    public boolean agregarTelefono(String numero, String tipo, int id_usuario) throws SQLException {
        //Primero creamos el comando sql 
        String comandoSQL = "SELECT id_cliente FROM Clientes WHERE id_usuario = ?";
        //declaramos una varibale para poder guardar el id del clinet
        int id_cliente;
        try (Connection conn = this.conexionBD.crearConexion(); PreparedStatement ps = conn.prepareStatement(comandoSQL)) {
            ...
            if (rs.next()) {
                ...
                throw new SQLException("No se encontro un cliente para este usuario");
            }
        }
        
        //Nos movemos a insertar el telefono 
        //Creamos el comando 
        String comandosql = "INSERT INT TelefonosClientes (id_cliente, telefono, tipo) VALUES (?,?,?)";
        try(Connection conn = this.conexionBD.crearConexion(); PreparedStatement ps = conn.prepareStatement(comandoSQL)) {
